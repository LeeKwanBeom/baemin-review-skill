...
진단 회차이므로 저장소 `SKILL.md`는 손대지 않았다. 이 커밋은 `audit/last-audit.md`만 추가한다. `audit/checklist.md`는 아래 "점검표 개정안"에서 사용자가 채택한 것만 다음 커밋에 반영한다.
...
| # | 내용 | 이유 | 우선순위 |
|---|---|---|---|
| 1 | `_scroll`에 자기 감속 감지: 라운드 실측이 800ms를 넘거나 `hidden===true`이면 예산을 다 쓰지 말고 즉시 반환하고 `throttled: true`를 실어 호출자가 Step 2의 "창 전면" 요청으로 되돌아가게 함 (인계 [B]) | [실측] 숨김 상태에서 12라운드×1000ms를 gained 0으로 소진. 결함 1의 코드 측 보완 | 1 |
| 2 | `_parse`가 건너뛴 게시중단 리뷰를 `window._blocked`에 기록하고 반환값에 `blocked`를 포함, `collected + blocked === expectedTotal`이면 정상으로 명시 | [실측] 오늘 190 = 189 + 1 이 즉시 확인됨. 없으면 매번 "1건 차이"를 놓고 추측하게 됨(어제 세션이 그랬음) | 2 |
| 3 | Step 3 적용 후 (a) 기간 다이얼로그가 닫혔는지, (b) `range`가 오늘 기준 30일 범위(예: `2026. 8. 10 ~ 2026. 9. 9`)와 일치하는지 코드로 판정 | [실측] 다이얼로그가 열려 있는 동안에도 `labelOk`(본문에 "최근 30일" 포함)가 true이고 range 정규식은 6개월 범위(`2026. 3. 10 ~`)를 잡는다. 적용 클릭이 무효가 되면 지금 코드는 `filterOk=true`·`expectedTotal=옛 기간 건수(1,453)`로 통과한다. 그 조건은 만들지 못해 결함이 아니라 개선안 | 3 |
| 4 | 사이트 현행화: (a) `DT`에 `'픽업'` 추가 (b) Step 2 `titleOk`를 본문 전체가 아니라 헤더 줄 `[음식배달] <매장명>`(뒤에 ` / `가 없는 줄) 또는 `<shopId> \|` 부제로 한정 | [실측] (a) 381건 중 3건이 `픽업`으로 시작(김치찜 2·참제육 1) — 지금은 3차 fallback으로 닉네임이 맞았을 뿐. (b) 곱도리 페이지 본문에 `[음식배달] 김치찜의 정석 / 찜·탕·찌개 14698107`(계정 매장 목록)이 있어 같은 계정 매장명은 어느 페이지에서나 `titleOk=true` | 4 |
| 5 | 검증 공백(점검표 D): 실행 전 자동 백업(`$HOME/skillwork/배민_백업_<시각>.xlsx`)을 Step 0에 넣고, Step 5 저장 후 파일을 다시 열어 행수·헤더를 확인하는 한 줄을 추가 | 스냅샷 방식이라 백업이 사실상 필수인데 절차에 없음. 저장 후 재확인 단계 없음 | 5 |
...
- 결함 1~4 중 사용자가 채택한 것이 저장소 `SKILL.md`에 반영됐고, 재패키징·재업로드로 설치본 md5가 바뀌었는지.
- 수정 후 실행에서 첫 탭의 `hidden` 값과 `setTimeout(250)` 실측(창 전면 조치 전/후).
- `collected + blocked === expectedTotal` 이 3매장에서 성립하는지(개선안 2 채택 시).
- 45초 CDP 타임아웃 재현 여부와 그때의 탭 숨김 경과 시간.
- 곱도리 닉네임 실패 1건(2026082400693614)의 카드 첫 줄 구조.
- 실측 못 한 것: 로그아웃 상태 접근 시 URL 패턴, `expectedTotal === null` 강제 시 흐름(코드상 4-3에서 ok:false로 떨어짐은 확인, 실행은 안 함).
